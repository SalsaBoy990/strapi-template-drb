...
async function importGlobal() {
  ...
  const source = await checkFileExistsBeforeUpload([video.source]);
  const videoCopy = { ...video };
  videoCopy.source = source;
...
async function importFooter() {
  ...
  const flag = await checkFileExistsBeforeUpload([footer.flag]);
  const developer = await checkFileExistsBeforeUpload([footer.developer]);
